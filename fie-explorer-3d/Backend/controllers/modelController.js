...
// ── GET / — público, lo usa el visor 3D ──────────────────────
...
async function create(req, res, next) {
  ...
  try {
    ...
    await writeAudit({
      ...
      new_values: {
        building_id, model_type, file_path, lod_level: lod_level??0,
        scale_x, scale_y, scale_z,
        rotate_x: rotate_x??0, rotate_y: rotate_y??0, rotate_z: rotate_z??0,
      },
    ...
    if (err.code === '23505' && err.constraint === 'uq_models_building_type_lod') {
      ...
      const lodLabel  = LOD_NAMES[lod_level ?? 0] ?? `LOD ${lod_level}`;
      const e = new Error(
        `Ya existe un modelo ${model_type} de resolución ${lodLabel} para este edificio. ` +
        `Elimina o desactiva el modelo existente antes de registrar uno nuevo.`
      );
...
async function update(req, res, next) {
  ...
  try {
    ...
    await writeAudit({
      ...
      old_values: {
        file_path: before.rows[0].file_path, is_active: before.rows[0].is_active,
        scale_x: before.rows[0].scale_x, scale_y: before.rows[0].scale_y, scale_z: before.rows[0].scale_z,
        rotate_x: before.rows[0].rotate_x, rotate_y: before.rows[0].rotate_y, rotate_z: before.rows[0].rotate_z,
      },
      new_values: {
        file_path: rows[0].file_path, is_active: rows[0].is_active,
        scale_x: rows[0].scale_x, scale_y: rows[0].scale_y, scale_z: rows[0].scale_z,
        rotate_x: rows[0].rotate_x, rotate_y: rows[0].rotate_y, rotate_z: rows[0].rotate_z,
      },
      ip_address: req.ip, user_agent: req.headers['user-agent'],
    });
    log(`✏️  ACTUALIZADO — modelo ${id} (rotate_y: ${rows[0].rotate_y}°)`);
    res.json(rows[0]);
  } catch (err) { next(err); }
}
