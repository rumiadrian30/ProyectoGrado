...
async function create(req, res, next) {
  ...
  try {
    ...
    await writeAudit({
      ...
      new_values: { building_id, model_type, file_path, lod_level: lod_level??0, scale_x, scale_y, scale_z },
      ip_address: req.ip, user_agent: req.headers['user-agent'],
    });
    log(`✅ CREADO — ${model_type} LOD${lod_level??0} → ${file_path}`);
    res.status(201).json(rows[0]);
  } catch (err) {
    // Violación de unicidad: ya existe un modelo con ese building + type + lod
...
async function update(req, res, next) {
  ...
  try {
    ...
    `, [file_path, file_size_mb, triangle_count, version, is_active,
        scale_x, scale_y, scale_z, offset_x, offset_y, offset_z, id]);
    await writeAudit({
